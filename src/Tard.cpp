...
void Tard::executar() {
    ...
    if (Keyboard::isKeyPressed(Keyboard::Numpad0) && ataquePronto) {
        atacando = true;
        totalT -= 0.5F;
        hitbox->getCorpoGraf()->setPosicao(corpo.getPosicao().x + (100.0F * static_cast<float>(this->lado)),
                                           corpo.getPosicao().y);
        ataquePronto = false;
        ladoAtaque = lado;
    ...
    if (atacando)
        hitbox->getCorpoGraf()->getAnimadora()->atualizarLinhasSequencial(pGG->getDt(), aDireita, Vector2u(9, 9),
                                                                          0.1F);
