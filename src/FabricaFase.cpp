...
FabricaFase::~FabricaFase() = default;

void FabricaFase::setJogadores(Vita *j1, Tard *pTard) {
    ...
    if (doisJogadores) {
        pTard = new Tard(pJogo->getGerenciador());
        pTard->setVidas(pTard->getVidas());
        pTard->setPontos(pTard->getPontos());
        pTard->setPosicao(Vector2f(pTard->getPosicao().x, -1000.0f));
    }
}
...
void FabricaFase::carregar() {
    ...
    int tipo = 0, id = 0, aux1 = 0, aux2 = 0, numJogadores = 1, i;
    Vector2f pos, tam;
